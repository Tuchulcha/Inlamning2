...
def register_activation_hooks(model):
    for layer in model.modules():
        if isinstance(layer, torch.nn.modules.Linear):  # Changed to Linear
            layer.register_forward_hook(
                lambda layer, input, output: writer.add_histogram(f"{layer.__class__.__name__}_activations", output)
            )
...
def train_model(model, train_loader, criterion, optimizer, num_epochs=10):
    model.train()
    register_activation_hooks(model)  # Register activation hooks // register_forward_hook
    register_gradient_hooks(model)    # Register gradient hooks
